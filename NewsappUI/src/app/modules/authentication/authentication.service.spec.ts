...
fdescribe("AuthenticationService", () => {
  ...
  it("should be created Authenticate Service", inject(
  ...
  it("should register user data", fakeAsync(() => {
    ...
    inject(
      ...
      (backend: HttpTestingController) => {
        ...
        expect(mockReq.request.url).toEqual(
          authService.authServiceEndpoint,
          "request url should match with json server api url"
        );

        expect(mockReq.request.method).toBe(
          "POST",
          "Should handle requested method type"
        );
